...
const RegisterScreen = () => {
	...
	const handleRegister = async (e) => {
		...
		if (emailInput.trim() === "") {
			setEmailError(
				"Por favor, introduzca un email válido (example@example.com)."
			);
			setLoading(false);
			return;
		}
					...
					<div className="row">
						<div className="col-12 col-md-6 offset-md-3 card-login">
							...
							<form onSubmit={handleRegister} className="py-4">
								...
								<div className="mt-3 text-white">
									...
									<div className="input-group">
										...
										>
											{showPassword ? (
												<i
													className="fa fa-eye-slash eye-color"
													aria-hidden="true"
												></i>
											) : (
												<i
													className="fa fa-eye eye-color"
													aria-hidden="true"
												></i>
											)}
										</button>
								...
								<Link
									to={`/login`}
									className="text-white text-center-important"
								>
									¿Ya tienes cuenta?
									<br />
									¡Inicia sesión aquí! 😁
